...
Func CanStore($item)
	...
	If $ModelID == $ITEM_DYES Then
		...
		Return $ExtraID == $ITEM_BLACK_DYE Or $ExtraID == $ITEM_WHITE_DYE)
    EndIf ;Dies
	...
	If $ModelID == $ITEM_LOCKPICK           		Then Return True ;Lockpicks
...
Func RetrieveIdentificationKit()
    ...
    Do
        ...
        RndSleep(500)
        $j = $j + 1
    Until FindIdentificationKit() Or $j = 3
    
    If $j = 3 Then 
        MsgBox(16, "Unexpected Error", "Unable to retrieve an Identification Kit")
        Exit
    EndIf
    RndSleep(500)
